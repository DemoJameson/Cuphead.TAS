...
public static class Manager {
    ...
    private static void FrameStepping() {
        if (!Game.IsLoading && States.HasFlag(States.Enable) && (States.HasFlag(States.FrameStep) || Hotkeys.FrameAdvance.Pressed || Hotkeys.PauseResume.Pressed)) {
    ...
    public static void SendStateToStudio() {
        if (UltraFastForwarding && Game.FrameCount % 13 > 0) {
            return;
        }
